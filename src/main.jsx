...
const VA={cliente:"",painel:"SOLARMAN",situacao:"",prioridade:"Média",status:"Pendente",responsavel:"",observacao:"",arquivo_url:""},VC={nome:"",contato:"",cidade:"",painel:"SOLARMAN",potencia:"",status:"Normal"},VG={data:"",horario:"",local:"",servico:"",equipe:"",status:"Agendado",tipo_os:"Manutenção",diagnostico:"",solucao:"",arquivo_url:""},VR={dia:"",cliente:"",atividade:"",resultado:"",responsavel:"",status:"Bem-sucedido",arquivo_url:""};
...
{aba==="agenda"&&<Area><Form title={edit?.tabela==="agenda"?"Editar tarefa":"Nova tarefa"} edit={edit?.tabela==="agenda"} cancel={cancelar} file={setArq} submit={e=>{e.preventDefault();salvar("agenda",serv,()=>setServ(VG))}}><Campo l="Data" type="date" v={serv.data} s={v=>setServ({...serv,data:v})}/><Campo l="Horário" type="time" v={serv.horario} s={v=>setServ({...serv,horario:v})}/><Campo l="Local/Cliente" v={serv.local} s={v=>setServ({...serv,local:v})}/><Select l="Tipo de OS" v={serv.tipo_os} o={["Manutenção","Instalação","Vistoria","Retorno","Emergência"]} s={v=>setServ({...serv,tipo_os:v})}/><Texto l="Serviço a fazer" v={serv.servico} s={v=>setServ({...serv,servico:v})}/><Campo l="Equipe/Técnico" v={serv.equipe} s={v=>setServ({...serv,equipe:v})}/><Select l="Status da OS" v={serv.status} o={STATUS_OS} s={v=>setServ({...serv,status:v})}/><Texto l="Diagnóstico técnico" v={serv.diagnostico} s={v=>setServ({...serv,diagnostico:v})}/><Texto l="Solução aplicada" v={serv.solucao} s={v=>setServ({...serv,solucao:v})}/></Form><Panel title="Agenda">{agenda.map(s=><div className="osCard" key={s.id}>
  <div className="osTop">
  ...
  {s.arquivo_url&&<a href={s.arquivo_url} target="_blank">Abrir anexo</a>}
